...
- **MQTT:** The publisher sends HA auto-discovery configs and state updates. The commander parses HA payloads into ECHONET SetC requests. A bridge device ("EchonetGO") tracks overall online/offline status. Note that EchonetGO is architecturally opposed to "blind" network-wide auto-discovery to prevent entity bloat and fragile mappings from unrecognized devices; instead, it performs targeted discovery only after a device is explicitly configured and its specific properties have been successfully polled and verified against a known specification.
...
2. **Override:** To test your changes locally, you can create a `specs/` directory in your config folder and copy the project's base specs into it. See the [Add-on Documentation](addon_echonetgo/DOCS.md#customizing-device-behavior) for details.
